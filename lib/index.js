...
function skewness( v, options ) {
	...
	if ( isTypedArrayLike( v ) ) {
		...
		return skewness6( out, v );
	}
	if ( isArrayLike( v ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return skewness4( v, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return skewness3( out, v, opts.accessor );
		}
		return skewness2( out, v );
	}
	return NaN;
